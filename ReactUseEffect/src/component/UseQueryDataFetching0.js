...
function UseQueryDataFetching1() {
   ...
   console.log("dataAlbums", dataAlbums, "statusAlbums", statusAlbums);
    ...
    const responsePosts = await fetch('https://jsonplaceholder.typicode.com/todos').then(response => response.json());
    ...
    <div className='float-container'>
        ...
        <div className='float-child'> 
            <ul>
                {
                    // <li> Posts </li>
                    dataPosts.map(post => <li key={post.id}>{post.title}</li> )
                }
            </ul>
        </div>        
    </div>
...
export default UseQueryDataFetching1